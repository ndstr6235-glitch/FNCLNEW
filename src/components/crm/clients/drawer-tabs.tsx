...
const TABS: { key: Tab; label: string; countKey?: "paymentCount" | "eventCount"; requireDocuments?: boolean; requireTickets?: boolean }[] = [
  { key: "overview", label: "Přehled" },
  { key: "payments", label: "Platby", countKey: "paymentCount" },
  { key: "events", label: "Události", countKey: "eventCount" },
  { key: "email", label: "Email" },
  { key: "tickets", label: "Požadavky", requireTickets: true },
...
export default function DrawerTabs({
  ...
  const visibleTabs = TABS.filter(
    (tab) =>
      (!tab.requireDocuments || showDocuments) &&
      (!tab.requireTickets || showTickets)
  );
