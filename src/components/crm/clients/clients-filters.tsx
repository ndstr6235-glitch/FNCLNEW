...
export default function ClientsFilters({
  ...
  outcomeCounts,
  totalCount = 0,
  currentOutcome: currentOutcomeProp,
}: ClientsFiltersProps) {
  ...
  const currentOutcome = currentOutcomeProp ?? searchParams.get("outcome") ?? "all";
  ...
  function handleOutcome(value: string) {
    router.push(buildUrl({ outcome: value }));
  }
        ...
        >
          ...
          {CALL_OUTCOMES.map((o) => (
            <option key={o.code} value={o.code}>
              {o.icon} {o.label} ({outcomeCounts?.[o.code] ?? 0})
            </option>
          ))}
        </select>
        <ChevronDown
