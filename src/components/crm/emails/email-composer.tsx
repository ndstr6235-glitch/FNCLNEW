...
import { generateSalutation, generateEmailDraft, improveEmailText } from "@/app/actions/crm/ai";
import { cn, fmtCZK } from "@/lib/crm/utils";
import { sendEmail } from "@/app/actions/crm/emails";
...
interface EmailComposerProps {
  ...
  clientId?: string;
  /** Pre-fill values from the Client record so broker doesn't re-type */
...
const TEAM_SIGNATURES = [
  ...
  },
  {
    ...
    email: "martin@puskinpartners.cz",
    phone: "",
  },
  {
    id: "admin",
...
export default function EmailComposer({
  ...
  // Smlouva finální — vše se zadává čistě v composeru, NEpředvyplňuje se z CRM
  ...
  // Reset subject when template changes — use template subject as-is,
  // without injecting client name (admin can edit manually if needed).
  useEffect(() => {
    if (selectedTemplate) {
      setSubjectOverride(selectedTemplate.subject);
    }
          ...
          {showContractFields && (
            <div className=" border-2 border-gold/30 bg-gold-pale p-3 space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                {/* Trvalá adresa klienta */}
                <div className="md:col-span-2">
                  <label className="block text-xs font-medium text-text-mid mb-1">
                    Trvalá adresa — ulice a č.p.
                  </label>
                  <input
                    type="text"
                    value={street}
