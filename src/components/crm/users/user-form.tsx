...
import { createUser, updateUser, type UserRow } from "@/app/actions/crm/users";
...
export default function UserForm({ open, onClose, editData }: UserFormProps) {
  ...
  const [dailyLeadQuota, setDailyLeadQuota] = useState("150");
  ...
  useEffect(() => {
    if (open) {
      ...
      setDailyLeadQuota(String(editData?.dailyLeadQuota ?? 150));
      setError("");
    }
  }, [open, editData]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const quotaNum = Number(dailyLeadQuota);
    const quota = Number.isFinite(quotaNum) && quotaNum >= 0 ? quotaNum : 25;
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Daily lead quota — only meaningful for brokers */}
        {role === "broker" && (
          <div>
            <label className="block text-xs font-medium text-text-mid mb-1">
              Denní kvóta leadů
